...
| `initialRenameTrigger` | `string` | `"first-input"` | Cuándo se dispara el primer cambio de nombre：<br><ul><li>`"first-input"` — después de enviar el primer prompt</li><li>`"first-agent-settled"` — después de que la primera ejecución LLM termine（después del primer evento `agent_settled`）。</li></ul> |
| `language` | `string` | `"en"` | Uno de：`en`、`es`、`de`、`fr`、`it`、`nl`、`pt`/`pt-BR`/`pt-PT`、`id`、`vi`、`tr`、`pl`、`uk`、`fa`、`ar`、`hi`、`zh`/`zh-CN`/`zh-Hans`、`zh-Hant`/`zh-TW`/`zh-HK`、`ja`、`ko`、`th` |
| `namingContextDepth` | `string` | `"recent-user-messages"` | Cuánta conversación enviar al LLM en cada prompt de cambio de nombre：<br><ul><li>`"first-user-message"` — solo el primer mensaje del usuario</li><li>`"recent-user-messages"` — primer mensaje del usuario más los últimos 3 mensajes del usuario</li><li>`"full-conversation"` — toda la conversación（usuarios，asistentes，llamadas a herramientas y resultados），limitada a ~60k caracteres manteniendo tanto el inicio（intención principal）como la cola más reciente</li></ul> |
| `namingModel` | `string` | `""` | Anulación `provider/modelId` para la llamada LLM de cambio de nombre。Ejemplo：`"openrouter/nvidia/nemotron-3-nano-30b-a3b:free"`。Deja vacío para usar el modelo y proveedor predeterminados de Pi。 |
| `namingStyle` | `string` | `"natural"` | El estilo de cambio de nombre utilizado para ambos nombres：<br><ul><li>`"natural"` — oración libre</li><li>`"slug"` — minúsculas con guiones。</li><li>`"topic-project"` — `<tema>｜<proyecto>`，proyecto derivado del directorio de trabajo actual</li></ul> |
| `replaceExistingName` | `string` | `"always"` | Cuándo sobrescribir un nombre de sesión Pi，nombre de ventana tmux，nombre de panel/ pestaña herdr，nombre de panel/ pestaña zellij existentes：<br><ul><li>`"always"` — siempre sobrescribir</li><li>`"never"` — nunca sobrescribir</li></ul> |
