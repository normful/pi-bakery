...
- Globalmente：`~/.pi/agent/pi-auto-name.json`（oppure `<PI_CODING_AGENT_DIR>/pi-auto-name.json` quando configurato）
- Percorso alternativo deprecato：`~/.config/pi-auto-name/config.json` viene ancora letto con la priorità più bassa
...
| `initialRenameTrigger`      | `string`  | <code>"first-input"</code>          | Quando viene attivata la prima rinomina：<br><ul><li><code>"first-input"</code> — dopo aver inviato il primo prompt</li><li><code>"first-agent-settled"</code> — dopo il completamento della prima esecuzione LLM（dopo il primo evento `agent_settled`）。</li></ul>                                                                                                                                                                                                                         |
| `language`                  | `string`  | `"en"`                              | Uno di：<code>en</code>、<code>es</code>、<code>de</code>、<code>fr</code>、<code>it</code>、<code>nl</code>、<code>pt</code>/<code>pt-BR</code>/<code>pt-PT</code>、<code>id</code>、<code>vi</code>、<code>tr</code>、<code>pl</code>、<code>uk</code>、<code>fa</code>、<code>ar</code>、<code>hi</code>、<code>zh</code>/<code>zh-CN</code>/<code>zh-Hans</code>、<code>zh-Hant</code>/<code>zh-TW</code>/<code>zh-HK</code>、<code>ja</code>、<code>ko</code>、<code>th</code>           |
| `namingContextDepth`        | `string`  | <code>"recent-user-messages"</code> | Quanta conversazione inviare al LLM in ogni prompt di rinomina：<br><ul><li><code>"first-user-message"</code> — solo il primo messaggio utente</li><li><code>"recent-user-messages"</code> — primo messaggio utente più gli ultimi 3 messaggi utente</li><li><code>"full-conversation"</code> — tutta la conversazione（utenti，assistenti，chiamate agli strumenti e risultati），limitata a ~60k caratteri mantenendo sia l'apertura（intento principale）che la coda più recente</li></ul> |
| `namingModel`               | `string`  | <code>""</code>                     | Sostituzione <code>provider/modelId</code> per la chiamata LLM di rinomina。Esempio：<code>"openrouter/nvidia/nemotron-3-nano-30b-a3b:free"</code>。Lascia vuoto per utilizzare il modello e fornitore Pi predefiniti configurati。                                                                                                                                                                                                                                                           |
| `namingStyle`               | `string`  | <code>"natural"</code>              | Lo stile di rinomina utilizzato per entrambi i nomi：<br><ul><li><code>"natural"</code> — frase libera</li><li><code>"slug"</code> — minuscolo con trattini。</li><li><code>"topic-project"</code> — `<argomento>｜<progetto>`，progetto derivato dalla directory di lavoro corrente</li></ul>                                                                                                                                                                                                |
| `replaceExistingName`       | `string`  | <code>"always"</code>               | Quando sovrascrivere un nome di sessione Pi esistente，nome finestra tmux，nome pannello/scheda herdr，nome pannello/scheda zellij：<br><ul><li><code>"always"</code> — sempre sovrascrivere</li><li><code>"never"</code> — non sovrascrivere mai</li></ul>                                                                                                                                                                                                                                   |
