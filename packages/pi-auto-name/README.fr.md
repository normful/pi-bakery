...
- Globalement：`~/.config/pi-auto-name/config.json`（respecte `XDG_CONFIG_HOME`）
...
| `initialRenameTrigger` | `string` | `"first-input"` | Quand le premier renommage est déclenché：<br><ul><li>`"first-input"` — après avoir envoyé le premier prompt</li><li>`"first-agent-settled"` — après la première exécution LLM est terminée（après le premier événement `agent_settled`）。</li></ul> |
| `language` | `string` | `"en"` | L'un de：`en`、`es`、`de`、`fr`、`it`、`nl`、`pt`/`pt-BR`/`pt-PT`、`id`、`vi`、`tr`、`pl`、`uk`、`fa`、`ar`、`hi`、`zh`/`zh-CN`/`zh-Hans`、`zh-Hant`/`zh-TW`/`zh-HK`、`ja`、`ko`、`th` |
| `namingContextDepth` | `string` | `"recent-user-messages"` | Quelle quantité de conversation envoyer au LLM dans chaque prompt de renommage：<br><ul><li>`"first-user-message"` — seulement le premier message utilisateur</li><li>`"recent-user-messages"` — premier message utilisateur plus les 3 derniers messages utilisateur</li><li>`"full-conversation"` — toute la conversation（utilisateurs，assistants，appels d'outils et résultats），limitée à ~60k caractères en conservant à la fois l'ouverture（intention principale）et la queue la plus récente</li></ul> |
| `namingModel` | `string` | `""` | Remplacement `provider/modelId` pour l'appel LLM de renommage。Exemple：`"openrouter/nvidia/nemotron-3-nano-30b-a3b:free"`。Laisse vide pour utiliser ton modèle et fournisseur Pi par défaut configuré。 |
| `namingStyle` | `string` | `"natural"` | Le style de renommage utilisé pour les deux noms：<br><ul><li>`"natural"` — phrase libre</li><li>`"slug"` — minuscules avec tirets。</li><li>`"topic-project"` — `<sujet>｜<projet>`，projet dérivé du répertoire de travail actuel</li></ul> |
| `replaceExistingName` | `string` | `"always"` | Quand remplacer un nom de session Pi，nom de fenêtre tmux，nom de panneau/onglet herdr，nom de panneau/onglet zellij existant：<br><ul><li>`"always"` — toujours remplacer</li><li>`"never"` — ne jamais remplacer</li></ul> |
