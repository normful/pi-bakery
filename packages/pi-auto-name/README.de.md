...
| `initialRenameTrigger` | `string` | `"first-input"` | Wann die erste Umbenennung ausgelöst wird：<br><ul><li>`"first-input"` — nachdem du den ersten Prompt gesendet hast</li><li>`"first-agent-settled"` — nachdem der erste LLM-Durchlauf abgeschlossen ist（nach dem ersten `agent_settled`-Ereignis）。</li></ul> |
| `language` | `string` | `"en"` | Eines von：`en`、`es`、`de`、`fr`、`it`、`nl`、`pt`/`pt-BR`/`pt-PT`、`id`、`vi`、`tr`、`pl`、`uk`、`fa`、`ar`、`hi`、`zh`/`zh-CN`/`zh-Hans`、`zh-Hant`/`zh-TW`/`zh-HK`、`ja`、`ko`、`th` |
| `namingContextDepth` | `string` | `"recent-user-messages"` | Wie viel vom Gespräch bei jedem Umbenennungs-Prompt an den LLM gesendet wird：<br><ul><li>`"first-user-message"` — nur die erste Benutzernachricht</li><li>`"recent-user-messages"` — erste Benutzernachricht plus die letzten 3 Benutzernachrichten</li><li>`"full-conversation"` — das gesamte Gespräch（Benutzer，Assistenten，Tool-Aufrufe und Ergebnisse），auf ~60k Zeichen begrenzt，wobei sowohl der Anfang（Kernabsicht）als auch das neueste Ende beibehalten werden</li></ul> |
| `namingModel` | `string` | `""` | `provider/modelId`-Überschreibung für den Umbenennungs-LLM-Aufruf。Beispiel：`"openrouter/nvidia/nemotron-3-nano-30b-a3b:free"`。Leer lassen，um dein konfiguriertes Pi-Standardmodell und den -Anbieter zu verwenden。 |
| `namingStyle` | `string` | `"natural"` | Der für beide Namen verwendete Benennungsstil：<br><ul><li>`"natural"` — freier Satz</li><li>`"slug"` — Kleinbuchstaben mit Bindestrich。</li><li>`"topic-project"` — `<Thema>｜<Projekt>`，Projekt abgeleitet vom aktuellen Arbeitsverzeichnis</li></ul> |
| `replaceExistingName` | `string` | `"always"` | Wann ein bestehender Pi-Sessionsname，tmux-Fenstername，herdr-Panel/-Tab-Name，zellij-Panel/-Tab-Name überschrieben werden soll：<br><ul><li>`"always"` — immer überschreiben</li><li>`"never"` — nie überschreiben</li></ul> |
