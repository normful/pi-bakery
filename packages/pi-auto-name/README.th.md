...
| `initialRenameTrigger` | `string` | `"first-input"` | การเปลี่ยนชื่อครั้งแรกจะถูกเรียกใช้เมื่อใด：<br><ul><li>`"first-input"` — หลังจากส่ง prompt แรก</li><li>`"first-agent-settled"` — หลังจาก LLM รันครั้งแรกเสร็จสิ้น（หลังจากเหตุการณ์ `agent_settled` ครั้งแรก）。</li></ul> |
| `language` | `string` | `"en"` | หนึ่งใน：`en`、`es`、`de`、`fr`、`it`、`nl`、`pt`/`pt-BR`/`pt-PT`、`id`、`vi`、`tr`、`pl`、`uk`、`fa`、`ar`、`hi`、`zh`/`zh-CN`/`zh-Hans`、`zh-Hant`/`zh-TW`/`zh-HK`、`ja`、`ko`、`th` |
| `namingContextDepth` | `string` | `"recent-user-messages"` | ปริมาณบทสนทนาที่ส่งไปยัง LLM ในแต่ละ prompt การเปลี่ยนชื่อ：<br><ul><li>`"first-user-message"` — เฉพาะข้อความแรกของผู้ใช้</li><li>`"recent-user-messages"` — ข้อความแรกของผู้ใช้บวกกับข้อความผู้ใช้ 3 ข้อความล่าสุด</li><li>`"full-conversation"` — บทสนทนาทั้งหมด（ผู้ใช้ ผู้ช่วย การเรียกใช้เครื่องมือและผลลัพธ์）จำกัดที่ ~60k อักขระ โดยคงทั้งส่วนเปิด（เจตนาหลัก）และส่วนท้ายล่าสุด</li></ul> |
| `namingModel` | `string` | `""` | การเขียนทับ `provider/modelId` สำหรับการเรียก LLM การเปลี่ยนชื่อ。ตัวอย่าง：`"openrouter/nvidia/nemotron-3-nano-30b-a3b:free"`。เว้นว่างเพื่อใช้โมเดลและผู้ให้บริการ Pi ค่าเริ่มต้นที่กำหนดค่าไว้。 |
| `namingStyle` | `string` | `"natural"` | รูปแบบการตั้งชื่อที่ใช้สำหรับทั้งสองชื่อ：<br><ul><li>`"natural"` — ประโยคอิสระ</li><li>`"slug"` — ตัวอักษรเล็กแยกด้วยขีดกลาง。</li><li>`"topic-project"` — `<หัวข้อ>｜<โปรเจกต์>` โปรเจกต์ที่ได้มาจากไดเรกทอรีการทำงานปัจจุบัน</li></ul> |
| `replaceExistingName` | `string` | `"always"` | เมื่อใดที่จะเขียนทับชื่อเซสชัน Pi ที่มีอยู่ ชื่อหน้าต่าง tmux ชื่อแผง/แท็บ herdr ชื่อแผง/แท็บ zellij：<br><ul><li>`"always"` — เขียนทับเสมอ</li><li>`"never"` — ไม่เคยเขียนทับ</li></ul> |
